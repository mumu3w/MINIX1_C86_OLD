...
void patch2(void)
{
  ...
  data_offset = 512L + (long)sizes[KERN].text_size;
...
void patch1(long all_size)
{
  ...
  ubuf[(SECTOR_SIZE/2) - 4] = sectors + 1;
...
void read_block(int blocknr, char *buf)
{
  ...
  fseek(image, (UDWORD)blocknr * SECTOR_SIZE, SEEK_SET);
